...
do
  ...
  vim.diagnostic.config {
    ...
    float = {
      ...
      format = function(diagnostic)
        if diagnostic.code then return string.format('%s (%s: %s)', diagnostic.message, diagnostic.source, diagnostic.code) end
        if diagnostic.source then return string.format('%s (%s)', diagnostic.message, diagnostic.source) end
    ...
    virtual_text = { severity = { min = vim.diagnostic.severity.WARN }, source = 'if_many', spacing = 2 }, -- Text shows up at the end of the line
  ...
  -- Better indenting
  vim.keymap.set('v', '<', '<gv', { desc = 'Reselect after indent' })
  vim.keymap.set('v', '>', '>gv', { desc = 'Reselect after indent' })
  ...
  vim.keymap.set('n', 'J', 'mzJ`z', { desc = 'Join lines but keep cursor' })
...
do
  ...
  -- vim.keymap.set('n', '<leader>sF', telescope_ignored_files, { desc = '[S]earch All [F]iles' })
  ...
  vim.keymap.set('n', '<leader>shc', builtin.commands, { desc = '[S]earch [hC]ommands' })
...
do
  ...
  --  They must be installed separately and will be enabled when available.
